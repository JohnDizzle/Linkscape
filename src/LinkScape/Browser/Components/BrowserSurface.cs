...
internal sealed class BrowserSurface : Component
{
    private Microsoft.UI.Xaml.Controls.Border? _host;
    private Microsoft.UI.Xaml.Input.TappedEventHandler? _browserContentTapped;

    public BrowserSurfaceController Controller { get; set; } = new();
    public BrowserTab SelectedTab { get; set; } = BrowserTab.CreateHome();
    public Action<string, Func<BrowserTab, BrowserTab>> UpdateTab { get; set; } = static (_, _) => { };
    public Action<string> OpenUriInNewTab { get; set; } = static _ => { };
    public bool IsCommandCenterOpen { get; set; }
    public Action DismissCommandCenter { get; set; } = static () => { };
    public Action<string, string> SetTitleFromCore { get; set; } = static (_, _) => { };
    public Action<string> SetAddressFromCore { get; set; } = static _ => { };
    public Action<bool, bool> SetNavAvailabilityFromCore { get; set; } = static (_, _) => { };
    public Action<bool> SetLoadingStateFromCore { get; set; } = static _ => { };
    public Action RefreshHistoryFromCore { get; set; } = static () => { };
        ...
        return Border(null)
            .Set(host =>
            {
                Controller.SetCallbacks(
                    SetTitleFromCore,
                    SetAddressFromCore,
                    SetNavAvailabilityFromCore,
                    SetLoadingStateFromCore,
                    RefreshHistoryFromCore);

                if (!ReferenceEquals(_host, host) &&
                    _host is not null &&
                    _browserContentTapped is not null)
                {
                    _host.Tapped -= _browserContentTapped;
                }

                _browserContentTapped ??= OnBrowserContentTapped;
                host.Tapped -= _browserContentTapped;
                host.Tapped += _browserContentTapped;
                _host = host;

                _ = Controller.ShowSelectedWebViewAsync(host, SelectedTab, UpdateTab, OpenUriInNewTab);
            })
    ...
    private void OnBrowserContentTapped(object? sender, Microsoft.UI.Xaml.Input.TappedRoutedEventArgs args)
    {
